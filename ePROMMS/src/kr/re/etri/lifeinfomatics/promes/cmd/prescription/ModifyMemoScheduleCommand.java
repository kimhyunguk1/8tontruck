...
public class ModifyMemoScheduleCommand implements Command {

	private String next = "";

	public ModifyMemoScheduleCommand(String next) {
		this.next = next;
	}
...
//    			DBManager.getInstance().updateQuery(connResource, sql);
			...
			{
				sql = "INSERT INTO MEMO (ID, PRESCRIPTION_ID, USER_ID, MEMO_CONTENT) ";
				sql += "VALUES (" + Util.getSqlString(memoId) + ", " + Util.getSqlString(prescriptionId) + ", " + Util.getSqlString(userId) + ", " + Util.getSqlString(memoContent) + ")";
				Log.out.debug(sql);
//				DBManager.getInstance().updateQuery(connResource, sql);
				DBManager.getInstance().updateQuery(sql);
			}
