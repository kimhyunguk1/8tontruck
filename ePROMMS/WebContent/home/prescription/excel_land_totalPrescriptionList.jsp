...
function exceloutput(){
	...
	//parent.changeTitle("������Ȳ");
	//var mainForm = document.form;
	//mainForm.cmd.value = 'loadTotalListExcelPage';
	//mainForm._prescriptionId.value = "a";
	//mainForm._userId.value = "b";	
	//mainForm._hospital.value = "c";	
	//mainForm.target = "prescriptionMainPage";
	//mainForm.submit();
	document.location.href="home/prescription/totalPrescriptionDown.jsp";
}
...
<input name="excel_output" type="button" value="���" onclick="javascript:exceloutput()"/>
<table width="100%"  border="0" bordercolor="#ececec" cellspacing="0" >
...
<%
		...
		if(i==0){
			for(int j=0; j<individualschedule.size(); j++){
				...
				if(j==0){
					%>
					<tr>
					<td width="40px" height="20px" nowrap >&nbsp;</td>
					</tr>
					<%
				}
				%>
				<tr>
				<td width="40px" height="20px" nowrap >&nbsp;<%=scheduleInfo.getAlarmStart().substring(4,6)%>/<%=scheduleInfo.getAlarmStart().substring(6,8)%></td>
				</tr>
				<%
			}
			%>
		...
		for(int j=0; j<individualschedule.size(); j++)
		{
			...
			if(j==0)
			{
				//String username =scheduleInfo.getUserName().substring(0, 1)+"**";
				String username =scheduleInfo.getUserName().toString();
%>
				<tr>
				<td width="40px" height="20px" nowrap >&nbsp; <%=username %></td>
				</tr>
<%
			}
...
%>			
			...
			<tr>
			<td width="20px" height="20px" nowrap align="center" <%=backcolor%>></td>
			</tr>
<%			
			}
		}		
