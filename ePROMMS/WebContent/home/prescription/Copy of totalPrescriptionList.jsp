...
$(document).ready(function(){
	...
	$.ajax({
		 ...
		 success:function(obj){			
		    ...
		    jQuery("#gridList").setGridWidth(750,false);
...
function showtip(nextTo, e, str1,str2,str3,str4,str5) {
	...
	tt.innerHTML = "이름 : " + name + "<br>";
	tt.innerHTML += "알람시작시간 : <b>" + alarmstart + "</b><br>";
	tt.innerHTML += "알람끝시간 : " + alarmend + "<br>";
	tt.innerHTML += "복용시간 : " + takentime + "<br>";
...
<%
		...
		if(i==0){
			for(int j=0; j<individualschedule.size(); j++){
				...
				%>
				<td width="20px"  ><%=scheduleInfo.getAlarmStart().substring(6,8) %></td>
				<%
			}
			%>
		...
		for(int j=0; j<individualschedule.size(); j++)
		{
			...
			if(scheduleInfo.getTakenStatus().equals("FINISHUNTAKEN")){
				tooltip = "title = \"이      름 : " +scheduleInfo.getUserName() +"&#10;알람시간 : "+DateEncode(scheduleInfo.getAlarmStart())+" ~ "+DateEncode(scheduleInfo.getAlarmEnd()) 
				+"&#10;복용시간 : 미복용"+"\"";				
			}
			else{
				tooltip = "title = \"이      름 : " +scheduleInfo.getUserName() +"&#10;알람시간 : "+DateEncode(scheduleInfo.getAlarmStart())+" ~ "+DateEncode(scheduleInfo.getAlarmEnd()) 
						+"&#10;복용시간 : "+DateEncode(scheduleInfo.getTakenTime())+"\"";
			}
...
%>		
				...
				<%}else{%>
					<td  width="9" height="9"><img src="<%=scheduleInfo.getImage()%>" <%=tooltip %>/></td>
<%				}		
			}
		}		
%>
