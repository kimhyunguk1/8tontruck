...
public class ModifyErrorCommand implements Command {

	private String next = "";

	public ModifyErrorCommand(String next) {
		this.next = next;
	}
	...
	public String execute(HttpServletRequest req) throws CommandException {
		try {
			...
			String errorContent = req.getParameter("_errorContent");
			String memoContent = req.getParameter("_memoContent");
			String takencheck = req.getParameter("_takencheck");
		
			errorContent = Util.toString(errorContent);
			memoContent = Util.toString(memoContent);
			takencheck = Util.toString(takencheck);
			...
			String errorCategory = req.getParameter("_errorCategory");
			...
			{
    			sql = "UPDATE MEMO SET";
    			sql += " ERROR_CONTENT = '" + errorContent + "', MEMO_CONTENT="+ Util.getSqlString(memoContent) +", CATEGORY = '" + errorCategory + "', TAKENCHECK = '" + takencheck + "'";
    			sql += " WHERE PRESCRIPTION_ID = " + Util.getSqlString(prescriptionId);
    			sql += " AND ID = " + Util.getSqlString(memoId);
    			Log.out.debug(sql);
//    			DBManager.getInstance().updateQuery(connResource, sql);
			...
			{
				sql = "INSERT INTO MEMO (ID, PRESCRIPTION_ID, USER_ID, CATEGORY, ERROR_CONTENT, MEMO_CONTENT,TAKENCHECK) ";
				sql += "VALUES (" + Util.getSqlString(memoId) + ", " + Util.getSqlString(prescriptionId) + ", ";
				sql += Util.getSqlString(userId) + ", " + Util.getSqlString(errorCategory) + ", " + Util.getSqlString(errorContent) +",";
				sql += Util.getSqlString(memoContent)+ ","+ Util.getSqlString(takencheck) + ")";
				Log.out.debug(sql);
//				DBManager.getInstance().updateQuery(connResource, sql);
				DBManager.getInstance().updateQuery(sql);
			}
